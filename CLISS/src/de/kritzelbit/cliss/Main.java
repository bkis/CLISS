...
public class Main {
	...
	public static void main(String[] args) throws UnsupportedAudioFileException, IOException, InterruptedException {
		...
		seq.addTrack("BD", new File("bd.wav"), new boolean[]{true, false, false, false, false, false, false, false});
		...
		while (!(line = input.nextLine()).equals("exit")){
			...
			if (line.startsWith("rnd")) seq.randomizePattern(line.split(" ")[1], Integer.parseInt(line.split(" ")[2]));
			if (line.startsWith("steps")) seq.setSteps(Integer.parseInt(line.split(" ")[1]));
		}
		input.close();
		seq.stop();
	}
